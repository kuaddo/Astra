...
import jp.shiita.astra.celestialsphere.DeviceOrientation
...
fun getDeviceDirection(latLng: LatLng, orientation: DeviceOrientation)
        : Vector3d {
    val tag = "getDirection"

    // 端末の初期方向を設定
    // 端末の画面の奥方向を端末の方向とする
    val initDirection = -createVector3dFromLatLng(1.0, latLng)

    // 回転軸の設定
    val theta = latLng.longitude
    val phi   = latLng.longitude
    val xAxis = createVector3dFromPolar(1.0, PI / 2, phi + PI / 2)
    val yAxis = createVector3dFromPolar(1.0, theta, phi + PI)
    ...
    val pitchRotated = rotateVector3dAroundAxis(initDirection, pitch, xAxis)
    return rotateVector3dAroundAxis(pitchRotated, roll, yAxis).normalized
}

/**
 * 極座標系から直交座標系3次元ベクトルを構成する関数
 */
fun createVector3dFromPolar(radius: Double,
                            theta: Double,
                            phi: Double): Vector3d {
 ...
 * @return 回転後のベクトル
 */
private fun rotateVector3dAroundAxis(target: Vector3d,
                                     rad: Double,
